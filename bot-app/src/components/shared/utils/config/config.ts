 ...
 * 2. If the access token is missing/expired, silently renew it with the stored
 *    refresh_token instead of forcing the user through the OAuth login screen
 *    again — this is what keeps the session (and both Real/Demo accounts)
 *    available automatically on return visits.
 * 3. Fetch OTP WebSocket URL from DerivWSAccountsService
 *
    ...
    try {
        ...
        if (!authInfo || !authInfo.access_token) {
            ...
            if (refreshToken && clientId) {
                try {
                    authInfo = await refreshAccessToken(refreshToken, clientId);
                } catch (refreshError) {
                    console.error('[DerivWS] Silent token refresh failed, clearing session:', refreshError);
                    clearAllAuthData();
                }
            }
        }
