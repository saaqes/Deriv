...
export default class GoogleDriveStore {
    ...
    constructor(root_store: RootStore) {
        makeObservable(this, {
            ...
            is_google_drive_configured: observable,
        ...
        // Google Drive is optional — only initialise the SDK when a client_id is configured.
        // (rsbuild's `define` injects literal `undefined` when GD_CLIENT_ID is unset.)
    ...
    // True only when GD_CLIENT_ID/GD_APP_ID/GD_API_KEY are present at build time.
    // Drives whether the Google Drive UI is shown at all (set in setKey()).
    ...
    setKey = () => {
        ...
        // All three credentials are required for the Picker to work, so the
        // feature is only "configured" when every one is present (see helper).
        this.is_google_drive_configured = isGoogleDriveConfigured();
    };
    ...
    async loadFile() {
        ...
        const xml_doc = await this.createLoadFilePicker(
            ...
            localize('Select a TradeLab Bot Strategy')
        );
